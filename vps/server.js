...
const ALLOWED_ORIGINS = new Set(
  (process.env.APP_ORIGINS || 'https://www.mirianbarreto.com.br,https://mirianbarreto.com.br,https://mirian-barreto-app.vercel.app,http://localhost:3000')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
);
...
async function processJob(jobId, inputPath, workoutId) {
  ...
  try {
    ...
    const { error: updateError } = await db.from('workouts')
      .update({ video_path: `${workoutId}.mp4` }).eq('id', workoutId);
    ...
    const { error: mediaError } = await db.from('workout_media').delete().eq('workout_id', workoutId);
